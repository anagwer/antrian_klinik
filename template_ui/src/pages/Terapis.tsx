...
export default function Terapis() {
  ...
  const [orderBy, setOrderBy] = useState('id_terapis');
  const [order, setOrder] = useState<'asc' | 'desc'>('asc');

  // Search filter
  const filteredList = terapisList.filter((item) => {
    const term = searchTerm.toLowerCase();
    return (
      String(item.id_terapis).includes(term) ||
      item.nama_terapis.toLowerCase().includes(term) ||
  ...
  const [selectedTerapis, setSelectedTerapis] = useState<any>(null);
  ...
  const fetchData = () => {
    setLoading(true);
    // Fetch both therapists and services
    Promise.all([
      fetch(`${API_BASE}/terapis?all=true`).then((res) => res.json()),
      fetch(`${API_BASE}/poliklinik?all=true`).then((res) => res.json()),
    ])
      .then(([terapisData, layananData]) => {
        if (terapisData.status === 'success') {
          setTerapisList(terapisData.data);
        } else {
          setErrorMsg(terapisData.message || 'Gagal mengambil data terapis/bidan.');
        }
  ...
  const handleOpenEdit = (item: any) => {
    setIsEdit(true);
    setIdTerapis(item.id_terapis);
    setNamaTerapis(item.nama_terapis);
  ...
  const handleOpenDelete = (item: any) => {
    setSelectedTerapis(item);
    setOpenDeleteDialog(true);
  };

  const handleSave = () => {
    if (!namaTerapis || !idLayanan) {
    ...
    const endpoint = isEdit ? 'terapis_update' : 'terapis_create';
    ...
    fetch(`${API_BASE}/${endpoint}`, {
      ...
      .then((data) => {
        if (data.status === 'success') {
          setSuccessMsg(isEdit ? 'Terapis/Bidan berhasil diperbarui.' : 'Terapis/Bidan berhasil ditambahkan.');
          setOpenDialog(false);
          fetchData();
        } else {
          setErrorMsg(data.message || 'Gagal menyimpan terapis/bidan.');
        }
      })
      .catch((err) => {
        console.error(err);
  ...
  const handleDelete = () => {
    ...
    fetch(`${API_BASE}/terapis_delete?id_terapis=${selectedTerapis.id_terapis}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.status === 'success') {
          setSuccessMsg('Terapis/Bidan berhasil dihapus.');
          setOpenDeleteDialog(false);
          fetchData();
        } else {
          setErrorMsg(data.message || 'Gagal menghapus terapis/bidan.');
        }
      })
      .catch((err) => {
        console.error(err);
    ...
    <Box p={3}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={4}>
        <Box>
          <Typography variant="h4" fontWeight="bold" color="primary.main">
            Manajemen Terapis & Bidan Klinik
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Kelola data terapis/bidan, jadwal praktek, dan penempatan layanan.
          </Typography>
        </Box>
        <Button
          variant="contained"
            ...
            <Table>
              <TableHead sx={{ bgcolor: 'grey.50' }}>
                <TableRow>
                  <TableCell style={{ fontWeight: 'bold' }}>
                    <TableSortLabel
                      active={orderBy === 'id_terapis'}
                      direction={orderBy === 'id_terapis' ? order : 'asc'}
                      onClick={() => handleRequestSort('id_terapis')}
                    >
                      ID
                    </TableSortLabel>
                  </TableCell>
                  <TableCell style={{ fontWeight: 'bold' }}>
                    <TableSortLabel
                      active={orderBy === 'nama_terapis'}
                      direction={orderBy === 'nama_terapis' ? order : 'asc'}
                      onClick={() => handleRequestSort('nama_terapis')}
                    >
                      Nama Terapis / Bidan
                    </TableSortLabel>
                  </TableCell>
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={6} align="center" sx={{ py: 3 }}>
                      Belum ada data terapis/bidan tersedia.
                    </TableCell>
                  </TableRow>
                ) : (
                  paginatedList.map((item) => (
                    <TableRow key={item.id_terapis} hover>
                      <TableCell>{item.id_terapis}</TableCell>
                      <TableCell sx={{ fontWeight: 'bold' }}>{item.nama_terapis}</TableCell>
                      <TableCell>
                        <Chip
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} fullWidth maxWidth="sm">
        <DialogTitle sx={{ fontWeight: 'bold' }}>
          {isEdit ? 'Edit Terapis / Bidan' : 'Tambah Terapis / Bidan Baru'}
        </DialogTitle>
        <DialogContent dividers>
          <Grid container spacing={2} pt={1}>
            <Grid item xs={12}>
              <TextField
                label="Nama Terapis / Bidan"
                fullWidth
                required
                value={namaTerapis}
                onChange={(e) => setNamaTerapis(e.target.value)}
                placeholder="Contoh: Bidan Aurelia, A.Md.Keb"
              />
            </Grid>
            <Grid item xs={12}>
      ...
      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        <DialogTitle sx={{ fontWeight: 'bold' }}>Hapus Terapis / Bidan</DialogTitle>
        <DialogContent>
          <Typography>
            Apakah Anda yakin ingin menghapus <strong>{selectedTerapis?.nama_terapis}</strong>?
          </Typography>
          <Typography color="error" variant="caption" display="block" mt={1}>
            Peringatan: Menghapus terapis/bidan ini juga akan menghapus data Antrean yang terhubung (Cascade Delete).
          </Typography>
        </DialogContent>
